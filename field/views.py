...
def model_field_form(request, field_type, model_field_id):
    ...
    context = {}
    if request.method == 'POST':
        form = FIELD_CHOICES[field_type](model=model, data=request.POST, prefix="%s_%d" % (field_type,model.id), instance=model_field.object)
        if form.is_valid():
            field = form.save()
            form = FIELD_CHOICES[field_type](model=model, instance=field, prefix="%s_%d" % (field_type,field.id))
    else:
        form = FIELD_CHOICES[field_type](model=model, instance=model_field.object,prefix="%s_%d" % (field_type,model_field.id))
